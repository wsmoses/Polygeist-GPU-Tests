...
all: $kernel ${kernel}_cu

$kernel: $kernel.c $kernel.h
	\${VERBOSE} \${CC} -o $kernel $kernel.c \${CFLAGS} -I. -I$utilityDir $utilityDir/polybench.c \${EXTRA_FLAGS}

${kernel}_cu: ${kernel}_host.cu ${kernel}_kernel.cu ${kernel}_kernel.hu
	\${VERBOSE} \${CUCC} -o ${kernel}_cu ${kernel}_host.cu ${kernel}_kernel.cu \${CUCFLAGS} -I. -I$utilityDir $utilityDir/polybench.c \${EXTRA_FLAGS}

clean:
	@ rm -f $kernel ${kernel}_cu
