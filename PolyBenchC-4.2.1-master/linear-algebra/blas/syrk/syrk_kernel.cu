...
__global__ void kernel0(double *A, double *C, double alpha, double beta, int n, int m)
{
        ...
        for (int c2 = 0; c2 < m; c2 += 32) {
          if (n >= t0 + c0 + 1)
            for (int c4 = t1; c4 <= ppcg_min(31, -c2 + 999); c4 += 16)
              shared_A_0[t0][c4] = A[(t0 + c0) * 1000 + (c2 + c4)];
          __syncthreads();
          ...
          if (n >= t0 + c0 + 1 && t0 + c0 >= t1 + c1)
            for (int c3 = 0; c3 <= ppcg_min(31, m - c2 - 1); c3 += 1) {
              private_C[0][0] += ((alpha * shared_A_0[t0][c3]) * A[(t1 + c1) * 1000 + (c2 + c3)]);
              if (t0 + c0 >= t1 + c1 + 16)
                private_C[0][1] += ((alpha * shared_A_0[t0][c3]) * A[(t1 + c1 + 16) * 1000 + (c2 + c3)]);
            }
          __syncthreads();
        }
        if (m <= 0) {
